...
function startCliProcess(): void {
  ...
  cliProcess.on('exit', (code, signal) => {
    console.error(`[astro-viber] CLI process exited (code=${code}, signal=${signal})`);

    for (const [, pending] of pendingRequests) {
      pending.reject(new Error(`CLI process exited unexpectedly (code ${code})`));
    }
...
function sendCommand(cmd: string, params: Record<string, any> = {}): Promise<any> {
...
app.whenReady().then(() => {
  ...
  ipcMain.handle('native:release-image', async (_event, imageId: string) => {
    try { await sendCommand('release', { imageId }); } catch { /* ignore */ }
  });

  ipcMain.handle('native:release-all-images', async () => {
    try { await sendCommand('releaseAll'); } catch { /* ignore */ }
  });
  ...
  ipcMain.handle(
    'native:save-image',
    async (_event, imageId: string, filePath: string, format: string, stretch?: any) => {
